...
void VulkanWindow::recreateSwapChain() {
    //width and height were changed in the event. That will be propogated throughout upon re-init
...
void VulkanWindow::setTexture(VkImage tex){
    textureImageView = createImageView(tex, VK_FORMAT_R8G8B8A8_SRGB);
    createDescriptorSets();
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
std::vector<const char*> VulkanWindow::getRequiredExtensions(){
    ...
    if(!SDL_Vulkan_GetInstanceExtensions(window, &sdl_extension_count, nullptr)){
        throw std::runtime_error("Could not get vulkan instance extension count.");
    }
    //sdl_extensions
    if(!SDL_Vulkan_GetInstanceExtensions(window, &sdl_extension_count, sdl_extensions)){
        throw std::runtime_error("Could not get vulkan instance extensions.");
    }
