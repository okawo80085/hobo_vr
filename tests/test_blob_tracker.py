...
def test_find_blobs_in_frame():
    with MockReadUpdates(0, [rgby_tracking_image_np]):
        with mock.patch("virtualreality.utilz.time.sleep"):
            ...
            assert (b.blobs['tracker1'] == np.asarray(
                [
                    [[196, 272]],
                    [[194, 274]],
                    [[193, 274]],
...
def test_solve_blob_poses():
    with MockReadUpdates(0, [rgby_tracking_image_np]):
        with mock.patch("virtualreality.utilz.time.sleep"):
            ...
            assert b.poses == {'tracker1':
                                   {'x': -0.34523609690742246, 'y': -0.28970498418772483, 'z': 1.4035486226748464}
                               }

            b.solve_blob_poses()

            assert b.poses == {'tracker1':
                                   {'x': -0.6575314351920128, 'y': 0.11647778625690625, 'z': 1.466077432585446}
                               }
